...
macro_rules! run {
    ($cmd: expr $(, $arg: tt)*) => {
        if cfg!(target_os = "linux") {
            ...
            let result = Command::new("sh").arg("-c").arg(cmd).output().map_err(to_problem!("Failed to execute command", 500))?;
            if result.status.success() {
                return Ok(())
            } else {
                return Err(Problem::new("Command failed", 500, String::from_utf8(result.stderr).unwrap()))
            }
        } else {
            let cmd = format!($cmd, $(,$arg)*);
